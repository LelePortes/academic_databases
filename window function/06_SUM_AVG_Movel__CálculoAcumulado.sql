...
SELECT             --faz a soma do faturamento do mês anterior com o atual (ele soma e tira média)
	...
	ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS 'Fat. Acumulado (em milhões)'
FROM Sales
...
SELECT
	...
	ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS 'Fat. Médio (em milhões)'
FROM Sales
ORDER BY [Data do Fechamento];
...
SELECT
	...
	Faturamento_MM AS 'Faturamento Total (em milhões)',
	SUM(Faturamento_MM) OVER(ORDER BY Data_Fechamento 
	ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS 'Fat. Acumulado (em milhões)'
FROM Sales
ORDER BY [Data do Fechamento]
